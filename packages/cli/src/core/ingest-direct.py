...
def process_file(file_path, db_conn=None):
    ...
    try:
        ...
        if db_conn:
            index_document(db_conn, file_path, file_path.name, title, aliases, category, tags, modified_at)
...
def main():
    ...
    for i, file_path in enumerate(files, 1):
        ...
        if i % 10 == 0:
            print(f"Progress: {i}/{total_files} files ({total_chunks} chunks uploaded, {indexed_files} indexed)")
